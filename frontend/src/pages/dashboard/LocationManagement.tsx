...
export default function LocationManagement() {
  const [stopCollection, setStopCollection] = useState<Stop[]>([]);
  const [pathCollection, setPathCollection] = useState<Path[]>([]);
  ...
  const handleCreateStop = async (data: StopCreate) => {
    try {
      ...
      setStopCollection([...stopCollection, newStop]);
  ...
  const handleEditStop = async (data: StopCreate) => {
    ...
    try {
      ...
      setStopCollection(stopCollection.map((s) => s.stop_id === editingStop.stop_id ? updatedStop : s));
      setEditingStop(null);
  ...
  const handleCreatePath = async (data: PathCreate) => {
    try {
      ...
      setPathCollection([...pathCollection, newPath]);
  ...
  const handleEditPath = async (data: PathCreate) => {
    ...
    try {
      ...
      setPathCollection(pathCollection.map((p) => p.path_id === editingPath.path_id ? updatedPath : p));
      setEditingPath(null);
  ...
  const handleDelete = async () => {
    try {
      if (deleteType === "stop") {
        ...
        setStopCollection(stopCollection.filter((s) => s.stop_id !== deletingId));
        ...
        setPathCollection(pathCollection.filter((p) => p.path_id !== deletingId));
  ...
  const filteredStops = stopCollection.filter((stop) =>
  ...
  return (
        ...
        <div className="lg:col-span-2">
          <Card className="p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button size="sm" className="bg-primary hover:bg-primary-dark" onClick={() => setIsStopModalVisible(true)}>
                + Add Stop
              </Button>
            </div>
        ...
        <div className="lg:col-span-3">
          <Card className="p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button size="sm" className="bg-primary hover:bg-primary-dark" onClick={() => setIsPathModalVisible(true)}>
                + Create Path
              </Button>
            </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                      {path.stops.map((pathStop, stopIndex) => {
                        const stopInfo = stopCollection.find(s => s.stop_id === pathStop.stop_id);
                        return (
    ...
    <CreateStopModal
      ...
      onOpenChange={setIsStopModalVisible}
    ...
    <CreatePathModal
      ...
      onOpenChange={setIsPathModalVisible}
      onSubmit={handleCreatePath}
      availableStops={stopCollection}
    />
