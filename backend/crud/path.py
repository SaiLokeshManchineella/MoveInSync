...
def get_path(db: Session, path_id: int) -> Optional[Path]:
    ...
    return db.query(Path).options(joinedload(Path.stops)).filter(Path.path_id == path_id).first()
...
def get_paths(
    ...
    """
    from sqlalchemy.orm import joinedload
    return db.query(Path).options(joinedload(Path.stops)).offset(skip).limit(limit).all()
...
def get_all_paths(db: Session) -> List[Path]:
    ...
    """
    from sqlalchemy.orm import joinedload
    return db.query(Path).options(joinedload(Path.stops)).all()
