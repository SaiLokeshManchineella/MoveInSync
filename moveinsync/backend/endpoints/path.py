...
from schemas import PathCreate, PathResponse, PathStopBase, StopResponse
...
def create_path(path_data: PathCreate, db: Session = Depends(get_db)):
    ...
    db_path = path.create_path(db, path_data)
    # Convert PathStop objects to PathStopBase for serialization
    stops_data = [
        {"stop_id": ps.stop_id, "stop_order": ps.stop_order}
        for ps in (db_path.stops if db_path.stops else [])
    ]
    return PathResponse(
        path_id=db_path.path_id,
        path_name=db_path.path_name,
        stops=[PathStopBase(**stop) for stop in stops_data]
    )


@router.get("/", response_model=List[PathResponse])
def get_paths(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    paths = path.get_paths(db, skip=skip, limit=limit)
    # Convert PathStop objects to PathStopBase for serialization
    result = []
    for p in paths:
        stops_list = list(p.stops) if p.stops else []
        stops_data = [
            {"stop_id": ps.stop_id, "stop_order": ps.stop_order}
            for ps in stops_list
            if ps.stop_id is not None and ps.stop_order is not None
        ]
...
def search_paths(
    ...
    paths = path.search_paths(db, search_term, skip, limit)
    result = []
    for p in paths:
        stops_list = list(p.stops) if p.stops else []
        stops_data = [
...
def get_paths_sorted(
    ...
    paths = path.get_paths_sorted_by_name(db, ascending, skip, limit)
    result = []
    for p in paths:
        stops_list = list(p.stops) if p.stops else []
        stops_data = [
...
def get_paths_containing_stop(stop_id: int, db: Session = Depends(get_db)):
    ...
    paths = path.get_paths_containing_stop(db, stop_id)
    result = []
    for p in paths:
        stops_list = list(p.stops) if p.stops else []
        stops_data = [
...
def get_paths_by_stop_count(
    ...
    paths = path.get_paths_by_stop_count(db, min_stops, max_stops)
    result = []
    for p in paths:
        stops_list = list(p.stops) if p.stops else []
        stops_data = [
...
def get_path(path_id: int, db: Session = Depends(get_db)):
    ...
    # Convert PathStop objects to PathStopBase for serialization
    stops_list = list(db_path.stops) if db_path.stops else []
    stops_data = [
        {"stop_id": ps.stop_id, "stop_order": ps.stop_order}
        for ps in stops_list
        if ps.stop_id is not None and ps.stop_order is not None
