...
export default function RouteConfiguration() {
  const [routeCollection, setRouteCollection] = useState<Route[]>([]);
  const [pathOptions, setPathOptions] = useState<Path[]>([]);
  const [isLoadingRoutes, setIsLoadingRoutes] = useState(true);
  const [isLoadingPathOptions, setIsLoadingPathOptions] = useState(true);
  ...
  const handleCreateRoute = async (data: RouteCreate) => {
    try {
      ...
      setRouteCollection([newRoute, ...routeCollection]);
  ...
  const handleEditRoute = async (data: RouteCreate) => {
    ...
    try {
      ...
      setRouteCollection(routeCollection.map(r => r.route_id === editingRoute.route_id ? updatedRoute : r));
  ...
  const handleDeleteRoute = async () => {
    ...
    try {
      ...
      setRouteCollection(routeCollection.filter(r => r.route_id !== deletingRouteId));
  ...
  const filteredRoutes = routeCollection.filter((route) =>
                        ...
                        onClick={async () => {
                          ...
                          if (newCapacity !== null && !isNaN(parseInt(newCapacity))) {
                            try {
                              ...
                              setRouteCollection(routeCollection.map(r => r.route_id === route.route_id ? updated : r));
                        ...
                        <DropdownMenuItem 
                          onClick={async () => {
                            ...
                            try {
                              ...
                              setRouteCollection(routeCollection.filter(r => r.route_id !== route.route_id));
                              toast({ 
