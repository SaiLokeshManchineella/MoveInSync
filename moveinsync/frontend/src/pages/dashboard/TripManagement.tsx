...
export default function TripManagement() {
  const [tripList, setTripList] = useState<DailyTrip[]>([]);
  const [deploymentList, setDeploymentList] = useState<Deployment[]>([]);
  const [activeTrip, setActiveTrip] = useState<DailyTrip | null>(null);
  const [activeTripRoute, setActiveTripRoute] = useState<Route | null>(null);
  const [activeTripStops, setActiveTripStops] = useState<Stop[]>([]);
  const [activeTripDeployments, setActiveTripDeployments] = useState<Deployment[]>([]);
  ...
  useEffect(() => {
    if (activeTrip) {
      fetchTripDeployments(activeTrip.trip_id);
      fetchAvailableResources(activeTrip.trip_id);
      fetchTripRouteAndStops(activeTrip.route_id);
    }
  }, [activeTrip]);
  ...
  const handleEditTrip = async (data: DailyTripCreate) => {
    ...
    try {
      const response = await fetch(`${API_URL}/trips/${activeTrip.trip_id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
      ...
      const updatedTrip = await response.json();
        setTripList(trips.map(t => t.trip_id === updatedTrip.trip_id ? updatedTrip : t));
              ...
              filteredTrips.map((trip) => (
                <div
                  ...
                  className={`p-3 rounded-lg border border-border hover:shadow-card-hover transition-smooth cursor-pointer ${
                    activeTrip?.trip_id === trip.trip_id ? "border-l-4 border-l-primary bg-primary/5" : ""
                  }`}
                >
        ...
        <Card className="p-6 lg:col-span-2">
          {!activeTrip ? (
            ...
            <>
              <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-2xl font-bold text-foreground">{activeTrip.display_name}</h2>
                ...
                <div className="flex items-center gap-6 text-sm text-muted-foreground mb-4">
                  <div>
                    Trip ID: {activeTrip.trip_id}
                  </div>
                  <div>Route ID: {activeTrip.route_id}</div>
                </div>

                <div className="grid grid-cols-4 gap-4 mb-6">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-foreground">
                      {activeTripDeployments.length}
                    </div>
                    <div className="text-xs text-muted-foreground">Vehicles</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-foreground">
                      {activeTrip.booking_status_percentage?.toFixed(0) || 0}%
                    </div>
                  ...
                  <div className="text-center">
                    <span className={`inline-block px-3 py-1 text-sm font-semibold rounded ${
                      getStatusColor(activeTrip.live_status) === "success"
                        ? "bg-success/10 text-success"
                        : getStatusColor(activeTrip.live_status) === "warning"
                        ? "bg-warning/10 text-warning"
                        : "bg-muted/10 text-muted-foreground"
                    }`}>
                      {activeTrip.live_status?.replace("_", " ").toUpperCase() || "UNKNOWN"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                ) : activeTripStops.length > 0 ? (
                  <RouteMap
                    stops={activeTripStops}
                    routeName={activeTripRoute?.route_display_name}
                    className="h-96"
                  />
                ) : (
                  <div className="bg-muted rounded-lg h-96 flex items-center justify-center">
              ...
              {activeTripDeployments.length === 0 ? (
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-semibold">Assigned Vehicles ({activeTripDeployments.length})</h3>
                    <Button 
                      size="sm"
                  ...
                  {activeTripDeployments.map((deployment) => {
